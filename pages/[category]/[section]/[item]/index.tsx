...
export const getStaticPaths: GetStaticPaths = async (ctx) => {
  const { organicAll } = await graphQLClientP.request(ITEM , {site: `${process.env.API_SITE}`})
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const { organicByCategoryAndSectionAndItem } = await graphQLClientP.request(PRODUCTS_BY_ITEM, {category: `${category}`, section: `${section}`, item: `${item}`, site: `${process.env.API_SITE}`})
  return {
    props: {
      items: organicByCategoryAndSectionAndItem,
      seo: {
        category: {
          name: res.name,
          href: res.href
